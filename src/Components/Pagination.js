...
const Pagination = (page) =>
{
   ...
   return (
      <div className="flex bg-white rounded-lg font-[Poppins] justify-center m-4">
         <button onClick={back} className="h-12 border-2 border-r-0 border-indigo-500
         ...
         </button>
         {
            pages.map((pg, i) => (
               ...
               w-12 ${cur === pg.page && 'bg-indigo-500 text-white'}`}>{pg.page}</button>
            ))
         }
         <button onClick={Next} className="h-12 border-2  border-indigo-500
